...
All retrieval operations use `POST /search` with form data. Their successful
response is the common response envelope with ranked frame records in `data`.
`model` is a configured registry key. The built-in fallback keys are
`siglip_alpha`, `siglip_beta`, and `metaclip`.
...
| --- | --- | --- | --- |
| `text` | `model`, `text` | shared fields | Text-to-frame search. |
| `image` | `model`, `image_path` | shared fields | Search by a readable image source. |
| `temporal` | `model`, `text` | shared fields, `main_event_index` | Text search with temporal-event handling. |
| `scroll` | `model`, `video_filter` | shared fields, `utility_feature` | Browse frames from a selected video/filter. |

`main_event_index` is an integer with default `0`. `utility_feature` defaults
to `shot` for scroll. `image_path` may be a path readable by the hub container,
a `data:image/...` URI, or an HTTP(S) image URL.
