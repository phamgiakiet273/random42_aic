...
class ClipSearchService:
    ...
    def _add_paths(self, records: list[dict]) -> list[dict]:
        ...
        for idx, record in enumerate(records):
            ...
            record["keyframe_id"] = get_true_frame_id(
                record["video_name"], record["keyframe_id"]
            )
            if "related_start_frame" in record:
                record["related_start_frame"] = get_true_frame_id(
                    record["video_name"], record["related_start_frame"]
                )
            if "related_end_frame" in record:
                record["related_end_frame"] = get_true_frame_id(
                    record["video_name"], record["related_end_frame"]
                )
        return records
