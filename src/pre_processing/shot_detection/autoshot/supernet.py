...
class TransNetV2Supernet(nn.Module):
    def __init__(
        ...
        self.Layer_1_8 = DilatedDCNNV2ABC(
            16 * 4, 16, multiplier=4, n_dilation=5, st_type="A"
        )
        self.Layer_2_8 = DilatedDCNNV2ABC(
            16 * 4, 32, multiplier=4, n_dilation=5, st_type="A"
        )
        self.Layer_3_8 = DilatedDCNNV2ABC(
            32 * 4, 32, multiplier=4, n_dilation=5, st_type="A"
        )
        ...
        self.fc1_0 = Linear_(
            in_features=in_features, out_features=D, bias=True, act="ReLU"
        )
        in_features += 1024
        self.fc1 = Linear_(
            in_features=in_features, out_features=D, bias=True, act="ReLU"
        )
        self.cls_layer1 = Linear_(
            in_features=1024, out_features=1, bias=True, act="Identity"
        )
        self.cls_layer2 = (
            Linear_(in_features=1024, out_features=1, bias=True, act="Identity")
            if use_many_hot_targets
            else None
        )
        self.frame_sim_layer = (
            FrameSimilarity(in_channels=448, inner_channels=101)
        ...
        self.dropout = (
            torch.nn.Dropout(p=1.0 - dropout_rate) if dropout_rate is not None else None
        )
...
class FrameSimilarity(nn.Module):
    def __init__(
        ...
        self.projection = Linear_(
            in_features=in_channels,
            out_features=similarity_dim,
            bias=use_bias,
            act="Identity",
        )
        self.fc = Linear_(
            in_features=inner_channels, out_features=output_dim, bias=True, act="ReLU"
        )
...
class ColorHistograms(nn.Module):
    def __init__(self, lookup_window=101, output_dim=128):
        ...
        self.fc = (
            Linear_(in_features=101, out_features=output_dim, bias=True, act="ReLU")
    ...
    def unsorted_segment_sum(self, data, segment_ids, num_segments):
        ...
        tensor = torch.zeros(*shape, device=self.device).scatter_add(
    ...
    def compute_color_histograms(self, frames):
        ...
        if no_channels == 3:
            frames_flatten = frames.reshape(
                shape=[batch_size * time_window, height * width, 3]
            )
        else:
            frames_flatten = frames.reshape(
                shape=[batch_size * time_window, height * width * 2, 3]
            )

        binned_values = get_bin(frames_flatten)
        frame_bin_prefix = (
            torch.arange(
                0, batch_size * time_window, dtype=torch.int32, device=self.device
            )
            << 9
        )
        ...
        histograms_normalized = histograms_normalized / torch.norm(
            histograms_normalized, dim=2, keepdim=True
        )
...
class ConvexCombinationRegularization(nn.Module):
    def __init__(
        ...
        raise Exception(
            "ConvexCombinationRegularization: should not init this class !!!"
        )
...
class DilatedDCNNV2(nn.Module):
    def __init__(
        ...
        self.batch_norm = (
            torch.nn.BatchNorm3d(num_features=filters * 4, eps=1e-3, momentum=0.1)
            if batch_norm
            else None
        )
        self.activation = activation
